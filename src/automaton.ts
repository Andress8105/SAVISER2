...
export type WorkflowAction =
  ...
  | 'REVISAR_RESULTADOS'
  | 'EMITIR_DIAGNOSTICO'
  | 'PRESCRIBIR_TRATAMIENTO'
  | 'DESPACHAR_MEDICAMENTO'
  | 'HOSPITALIZAR'
  | 'PROGRAMAR_CIRUGIA'
  | 'RECUPERAR'
  | 'DAR_SEGUIMIENTO'
...
export class HospitalAutomaton {
  private currentState: WorkflowState;
  private history: WorkflowHistory[];

  constructor(initialState: WorkflowState = 'INICIO') {
  ...
  getAvailableActions(): WorkflowTransition[] {
  ...
  performAction(
    action: WorkflowAction,
    metadata?: Record<string, unknown>
  ): { success: boolean; newState?: WorkflowState; error?: string } {
  ...
  setState(state: WorkflowState, history?: WorkflowHistory[]): void {
